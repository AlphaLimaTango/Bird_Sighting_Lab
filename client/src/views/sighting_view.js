...
SightingView.prototype.render = function (sighting) {
  ...
  const editButton = this.createEditButton(sighting._id);
  sightingContainer.appendChild(editButton);
...
SightingView.prototype.createEditButton = function (sightingId) {
  const button = document.createElement('button');
  ...
  button.addEventListener('click', (evt) => {
    PubSub.publish('SightingView:sighting-edit-clicked', evt.target.value);
  });
  return button;
};
